...
import random
import numpy as np
from experiments.exp_online_xy_combined import Exp_Combined
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Unified Adapter Experiment (Add, Mul, Affine)')
    ...
    parser.add_argument('--data_path', type=str, default='./datasets/', help='data csv file')
    parser.add_argument('--features', type=str, default='M',
                        help='forecasting task, options:[M, S, MS]; M:multivariate predict multivariate, S:univariate predict univariate, MS:multivariate predict univariate')
    parser.add_argument('--target', type=str, default='OT', help='target feature in S or MS task')
    parser.add_argument('--freq', type=str, default='h',
                        help='freq for time features encoding, options:[s:secondly, t:minutely, h:hourly, d:daily, b:business days, w:weekly, m:monthly], you can also use more detailed freq like 15min or 3h')
    parser.add_argument('--checkpoints', type=str, default='./AdaCali/checkpoints/', help='location of model checkpoints')
    ...
    parser.add_argument('--delta', type=float, default=0.1, help='delta for controlling adapter output scale')
    ...
    modes_to_run = []
    if args.adapter_mode == 'all':
        modes_to_run = ['add', 'mul', 'affine']
    else:
        modes_to_run = [args.adapter_mode]

    # Initialize Experiment
    # Note: We initialize generic arguments here. 
    # Exp_Combined needs to know the specific mode for building the model if the model architecture depended on it,
    # but since our PostProcessingNet is built dynamically in post_train(), we can reuse the same Exp instance 
    # OR create a new one for each mode.
    # HOWEVER, Exp_Combined loads the base model from 'checkpoint.pth' which is common.
    # To efficiently reuse the base model, we should implement the loop INSIDE Exp_Combined?
    # Or just instantiate Exp_Combined once and call a method to run adapters?
    
    # Current design: Exp_Combined takes args.
    # We will instantiate it once.
    # But wait, Exp_Combined.__init__ calls `self.model_dict...Model(self.args)`.
    # This loads/builds the BASE model structure.
    # Then `exp.train(setting)` trains the BASE model if it doesn't exist.
    
    # We want to train base model ONCE.
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            exp = Exp_Combined(args, adapter_mode='add')
            
            # 1. Base Model Training (or Skip if exists)
            ...
            for mode in modes_to_run:
                print(f"\n=====================================================")
                print(f"Running Adapter Mode: {mode}")
                print(f"=====================================================")
                
                # Update mode in current experiment instance
