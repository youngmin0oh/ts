...
class Buffer():
    def __init__(self, seq_len, pred_len):
        ...
        self.max_size = seq_len + pred_len
        self.batch_xy = None
        
    def __call__(self, batch_y):
        
        if self.batch_xy is None:
            self.batch_xy = batch_y[:,:1,:]
        else:
            self.batch_xy = torch.cat((self.batch_xy, batch_y[:,:1,:]),1)
        if self.batch_xy.size(1) < self.max_size:
            return None, None
        else:
            batch_x = self.batch_xy[:,:self.seq_len,:]
            batch_y = self.batch_xy[:,self.seq_len:,:]
            self.batch_xy = self.batch_xy[:,1:,:]
            return batch_x, batch_y


def adjust_learning_rate_v2(optimizers, epoch, args):
...
class EarlyStoppingV2:
    ...
    def __call__(self, val_loss, model, path, name='checkpoint.pth'):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(val_loss, model, path, name=name)
    ...
    def save_checkpoint(self, val_loss, model, path, name='checkpoint.pth'):
        ...
        for i,m in enumerate(model):
            torch.save(m.state_dict(), path + '/{}_'.format(i) + name)
