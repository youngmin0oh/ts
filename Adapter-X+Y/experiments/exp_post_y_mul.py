    ...
    def vali_post(self, post_net, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                outputs = outputs[:, -self.args.pred_len:, f_dim:]
                action = post_net(outputs.view(outputs.shape[0],-1))
        ...
        for episode in range(num_episodes):
            ...
            with tqdm(total=len(data_loader), desc='Iteration %d' % episode) as pbar:
                for step, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
                    ...
                    y_state = y_rl.view(y_rl.shape[0],-1)
                    action = post_net(y_state)

                    #loss_pred = criterion(y_pred.detach(), batch_y)
                    loss = criterion(action.view(*batch_y.shape)*y_rl, batch_y) # -(loss_pred - loss_rl)#*1000
                    loss_list.append(loss.item())
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(post_net.parameters(), 0.01)
                    post_optim.step()
            ...
            adjust_learning_rate(post_optim, episode + 1, self.args)   
