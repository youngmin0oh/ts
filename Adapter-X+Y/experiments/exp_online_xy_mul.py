...
class Exp_Decom(Exp_Basic):
    ...
    def post_train(self, setting, vali_set = False):
        ...
        for episode in range(self.args.num_episodes):
            ...
            if not vali_set:
                ...
                path = os.path.join(self.args.checkpoints, setting) 
                early_stopping(val_loss, (post_net_x, post_net_y), path, name='post_ckpt_{}_mul.pth'.format(vali_set))
                if early_stopping.early_stop:
                    print("Early stopping")
                    break
    ...
    def test2(self, setting, continue_train=False, post_process=False, vali_set=False, post_noise = False, online=False, visual_ts=False):
        ...
        if post_process:
            ...
            post_net_x.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, '0_post_ckpt_{}_mul.pth'.format(vali_set))))
            ...
            post_net_y.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, '1_post_ckpt_{}_mul.pth'.format(vali_set))))
