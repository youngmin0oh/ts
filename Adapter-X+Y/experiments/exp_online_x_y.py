...
class Exp_Decom(Exp_Basic):
    ...
    def post_train(self, setting, vali_set = False):
        ...
        post_net_x.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'post_ckpt_{}.pth'.format(vali_set))))
        post_net_x.eval()
        ...
        for episode in range(self.args.num_episodes):
            ...
            adjust_learning_rate(self.post_optim_y, episode + 1, self.args)
    ...
    def test2(self, setting, continue_train=False, post_process=False, vali_set=False, post_noise = False, online=False, visual_ts=False):
        ...
        if post_process:
            ...
            post_net_x.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, '0_post_ckpt_{}.pth'.format(vali_set))))
            ...
            post_net_y.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, '1_post_ckpt_{}.pth'.format(vali_set))))
        ...
        criterion = self._select_criterion()
        buffer = Buffer(self.args.seq_len, self.args.pred_len)
        self.post_optim_x = torch.optim.Adam(post_net_x.parameters(), lr=0.00001)
        self.post_optim_y = torch.optim.Adam(post_net_y.parameters(), lr=0.00001)
