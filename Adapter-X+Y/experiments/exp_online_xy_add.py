...
class Exp_Decom(Exp_Basic):
    ...
    def vali_post(self, post_net_x, post_net_y, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                infos = post_net_x(x_state)
                batch_x = batch_x + infos.view(*batch_x.shape) 
                
                outputs, batch_y = self._get_preds(batch_x, batch_y, batch_x_mark, batch_y_mark, i)
                
                action = post_net_y(outputs.reshape(outputs.shape[0],-1))
    ...
    def post_train(self, setting, vali_set = False):
        ...
        for episode in range(self.args.num_episodes):
            ...
            if not vali_set:
                ...
                path = os.path.join(self.args.checkpoints, setting) 
                early_stopping(val_loss, (post_net_x, post_net_y), path, name='post_ckpt_{}.pth'.format(vali_set))
                if early_stopping.early_stop:
                    print("Early stopping")
                    break
    ...
    def test2(self, setting, continue_train=False, post_process=False, vali_set=False, post_noise = False, online=False, visual_ts=False):
        ...
        if post_process:
            print('x_post_ckpt_{}.pth ...'.format(vali_set))
            post_net_x = PostProcessingNet(self.args.seq_len * self.args.enc_in, 
                                         self.args.d_model, 
                                         self.args.seq_len * self.args.enc_in,
                                            self.args.delta
                                         ).to(self.device)
            post_net_x.load_state_dict(torch.load(os.path.join(self.args.checkpoints + setting, '0_post_ckpt_{}.pth'.format(vali_set))))
            
            print('y_post_ckpt_{}.pth ...'.format(vali_set))
            post_net_y = PostProcessingNet(self.args.seq_len * self.args.enc_in, 
                                         self.args.d_model, 
                                         self.args.pred_len * self.args.enc_in,
                                            self.args.delta
                                         ).to(self.device)
            post_net_y.load_state_dict(torch.load(os.path.join(self.args.checkpoints + setting, '1_post_ckpt_{}.pth'.format(vali_set))))
        ...
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        buffer = Buffer(self.args.seq_len, self.args.pred_len)
        self.post_optim_x = torch.optim.Adam(post_net_x.parameters(), lr=0.000001)
        self.post_optim_y = torch.optim.Adam(post_net_y.parameters(), lr=0.000001)
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
            ...
            if online:
                ...
                if past_x is not None:
                    ...
                    infos = post_net_x(past_state)
                    past_x = past_x + infos.view(*past_x.shape)
                    
                    pred_past_y, _ = self._get_preds(past_x, past_y, None, None, i)
                    
                    action = post_net_y(pred_past_y.reshape(pred_past_y.shape[0],-1))
                    pred_past_y = pred_past_y + action.view(*pred_past_y.shape) 
                    
                    loss = criterion(pred_past_y, past_y)
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(post_net_x.parameters(), 0.0001)
                    torch.nn.utils.clip_grad_norm_(post_net_y.parameters(), 0.0001)
                    self.post_optim_x.step()       
                    self.post_optim_y.step()       
            
            _loss = criterion(outputs, batch_y)
            total_loss.append(_loss.item())
            _mae_loss = F.l1_loss(outputs, batch_y)
            total_mae_loss.append(_mae_loss.item())
        ...
        print('total_loss:{0:4f}, total_mae_loss:{1:4f}'.format(np.mean(total_loss), np.mean(total_mae_loss)))
        # # result save
        # folder_path = './results/' + setting + '/'
        # if not os.path.exists(folder_path):
        #     os.makedirs(folder_path)
