...
from utils.tools import EarlyStopping, adjust_learning_rate, visual, moving_avg
...
class PostProcessingNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x1 = self.fc3(x)
        return torch.tanh(x1) * 0.1



class PostY(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        super(PostY, self).__init__()
        self.fc1 = torch.nn.Linear(state_dim, hidden_dim)
        self.bn1 = torch.nn.BatchNorm1d(hidden_dim)
        self.fc2 = torch.nn.Linear(hidden_dim, hidden_dim)
        self.bn2 = torch.nn.BatchNorm1d(hidden_dim)
...
class Exp_Decom(Exp_Basic):
        ...
        for episode in range(self.args.num_episodes):
            ...
            with tqdm(total=len(data_loader), desc='Iteration %d' % episode) as pbar:
                for step, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
                    ...
                    y_rl = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                    y_rl = y_rl[:, -self.args.pred_len:, f_dim:]

                    loss = criterion(y_rl, batch_y)
    ...
    def vali_post(self, post_net, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                y_rl = outputs[:, -self.args.pred_len:, f_dim:]
                
                
                mean = y_rl.mean(1, keepdim=True)
                std = torch.sqrt(torch.var(y_rl- mean, dim=1, keepdim=True, unbiased=False) + 1e-5)
                y_rl = (y_rl - mean) / std
                
                action = post_net(y_rl.view(y_rl.shape[0],-1))
                action = action.view(*y_rl.shape)
                action = (action * std) + mean
                
                pred = action.detach().cpu()
                true = batch_y.detach().cpu()
    ...
    def post_train(self, setting, vali_set = False):
        ...
        post_net = PostY(self.args.pred_len * self.args.enc_in, 
                                         self.args.d_model, self.args.pred_len * self.args.enc_in).to(self.device)
        post_optim = torch.optim.Adam(post_net.parameters(), lr=self.args.post_train_lr)
        ...
        for episode in range(num_episodes):
            ...
            with tqdm(total=len(data_loader), desc='Iteration %d' % episode) as pbar:
                for step, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
                    ...
                    y_rl = self.model(batch_x, dec_inp)
                    y_rl = y_rl[:, -self.args.pred_len:, f_dim:]
                    ...
                    loss = criterion(action.view(*batch_y.shape), batch_y) # -(loss_pred - loss_rl)#*1000
                    loss_list.append(loss.item())
                    loss.backward()
                    post_optim.step()
            ...
            if not vali_set:
                val_loss = self.vali_post(post_net, val_loader, criterion)
                print('val_loss=%.4f' % val_loss)
               
            test_loss = self.vali_post(post_net, test_loader, criterion)
            print('test_loss=%.4f' % test_loss)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if post_process:
                    ...
                    infos = post_net(x_state)#/20 #(batch_x.max() - batch_x.min())
                    batch_x = batch_x + infos.view(*batch_x.shape)
                if post_noise:
                    infos = torch.randn_like(batch_x)/20#(batch_x.max() - batch_x.min())
